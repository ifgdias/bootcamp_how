...
class TestMercadoBitcoinApi:
    ...
    @patch("requests.get")
    @patch("mercadobitcoinapi.apis.MercadoBitcoinApi._get_endpoint", return_value="valid_endpoint")
    def test_get_data_requests_is_called(self, mock_get_endpoint, mock_requests_get, fixture_mercadobitcoin_api):
        fixture_mercadobitcoin_api.get_data()
        mock_requests_get("valid_endpoint")

    @patch("requests.get", side_effect=mocked_requests_get)
    @patch("mercadobitcoinapi.apis.MercadoBitcoinApi._get_endpoint", return_value="valid_endpoint")
    def test_get_data_with_valid_endpoint(self, mock_get_endpoint, mock_requests_get, fixture_mercadobitcoin_api):
        actual = fixture_mercadobitcoin_api.get_data()
        expected = {"foo":"bar"}
        assert actual == expected

    @patch("requests.get", side_effect=mocked_requests_get)
    @patch("mercadobitcoinapi.apis.MercadoBitcoinApi._get_endpoint", return_value="invalid_endpoint")
    def test_get_data_with_valid_endpoint(self, mock_get_endpoint, mock_requests_get, fixture_mercadobitcoin_api):
